...
$$\theta_i\sim N(X\mu, \phi s_i^2 + \tau^2)$$
$\phi\neq0$
...
# mu: (n-1)-dimensional vector, true effect sizes
...
generate_2arm <- function(X, mu, se, phi = 1, tau = 0, ref = "ref") {
...
for (i in 1:nsim){
  ...
  cover_common <- cover_common + min(mu <= net$upper.common[1,-1])* 
                                    min(mu >= net$lower.common[1,-1])
  cover_random <- cover_random + min(mu <= net$upper.random[1,-1])* 
                                    min(mu >= net$lower.random[1,-1])
    
  taus <- c(taus, net$tau)
}
  
variance <- apply(estTE, 1, var) #take variance of each row
bias <- rowMeans(estTE-mu)/mu
MSE <- rowMeans((estTE-mu)^2)
...
```{r}
fitme <- lm(data2$TE ~ X-1, weights = se^2)
summary(fitme)
```
```{r}
